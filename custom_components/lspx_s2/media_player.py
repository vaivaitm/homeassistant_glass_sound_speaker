"""Support for LSPX-S2 media devices."""
...
async def async_setup_platform(
    ...
    _LOGGER.error(
        "Configuring LSPX-S2 through media_player platform is no longer supported."
        " Convert to lspx_s2 platform or UI configuration"
    )
...
async def async_setup_entry(
    ...
    try:
        ...
        _LOGGER.debug("Unable to get methods from device: %s", ex)
        raise PlatformNotReady from ex

    lspx_entity = LspxEntity(name, device)
    async_add_entities([lspx_entity], True)

class LspxEntity(MediaPlayerEntity):
    """Class representing an LSPX-S2 device."""
